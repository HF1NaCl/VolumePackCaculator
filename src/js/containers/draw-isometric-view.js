...
import { ISO_CONTAINER_ID, GRID_MARGIN, FLOOR_COLORS } from './settings';
...
export function createGrid({ width, depth }) {
  ...
  const step = Math.max(1, Math.ceil(Math.max(width, depth) / 100));

  for (let index = 0; index <= width; index += step) {
    gridPath.moveTo(index, 0, 0).lineTo(index, depth, 0);
  }

  for (let index = 0; index <= depth; index += step) {
...
export function createBox({ right, left, top, width, depth, height, colors }) {
...
export function fitIsoCanvasToContent(container, padding = 12) {
